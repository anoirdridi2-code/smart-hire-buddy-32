/* Live job retrieval and strict professional-family filtering. */
...
async function getJson<T>(url: string, ms = 15000): Promise<T | null> {
  ...
  try { const res = await fetch(url, { signal: controller.signal, headers: { Accept: "application/json", "User-Agent": "Karriera/1.0" } }); if (!res.ok) return null; return (await res.json()) as T; }
...
const SYNONYMS: Record<string, string[]> = {
  ...
  automatisme: ["automation", "automation technician", "automation engineer", "plc", "sps", "scada", "controls"], automatique: ["automation", "control engineer", "plc", "controls"],
  ...
  controle: ["controls engineer", "control systems", "control technician", "leittechnik"], automaticien: ["automation technician", "automation engineer", "plc engineer", "automatisierungstechniker"],
  energie: ["energy engineer", "electrical energy", "photovoltaic", "solar", "renewable"], robotique: ["robotics", "robotics technician", "robotics engineer"],
...
const INDUSTRIAL_STRONG = [
  "automation", "automatisierung", "plc", "sps", "scada", "controls", "industrial maintenance", "maintenance technician", "maintenance engineer",
  "electrical technician", "electrical engineer", "electrical engineering", "electromechanical", "electrotechnics", "instrumentation", "control systems",
  "robotics", "cnc", "hvac", "hydraulic", "pneumatic", "electrician", "electrical maintenance", "automaticien", "automatisme", "electrotechnicien",
  "electricien", "technicien maintenance", "technicien electricite", "ingenieur maintenance", "ingenieur automatisme", "ingenieur electrique", "technicien electromecanique", "technicien instrumentation",
];
...
function expandKeywords(sources: string[]): string[] {
  ...
  for (const source of sources) { const norm = normalize(source); for (const [key, syns] of Object.entries(SYNONYMS)) if (norm.includes(key.slice(0, 6))) syns.forEach((s) => out.add(normalize(s))); for (const token of norm.split(/[^a-z0-9+#.-]+/)) if (token.length >= 4) out.add(token); }
...
const INDUSTRIAL_FAMILIES = ["industrie", "electricite", "automatisme", "maintenance", "electrotechnique", "electromecanique", "instrumentation", "controle", "robotique", "energie", "mecanique", "industrial", "automation", "electrical", "electromechanical", "electrotechnics", "manufacturing", "plc", "scada", "controls"];
function profileHasIndustrialFamily(text: string): boolean { return INDUSTRIAL_FAMILIES.some((x) => normalize(text).includes(normalize(x).slice(0, 6))); }
function excludedFamily(text: string): string | null { const t = normalize(text); for (const [family, markers] of Object.entries(EXCLUDED_FAMILIES)) if (markers.some((m) => t.includes(normalize(m)))) return family; return null; }
function isStrongIndustrialTitle(title: string): boolean { return hasAny(title, INDUSTRIAL_STRONG); }
function industrialSignalCount(text: string): number { const t = normalize(text); return INDUSTRIAL_STRONG.filter((m) => t.includes(normalize(m))).length; }
function relevance(job: LiveJob, terms: string[]): number { const title = normalize(job.title); const body = normalize(`${job.description} ${job.location ?? ""}`); let score = 0; for (const term of terms) { if (STOP.has(term)) continue; if (title.includes(term)) score += 5; else if (body.includes(term)) score += 1; } if (isStrongIndustrialTitle(job.title)) score += 10; return score; }

/** Shared predicate used both for fresh API results and for cleaning stale DB rows. */
export function isProfessionallyRelevantLiveJob(job: Pick<LiveJob, "title" | "description">, profileSources: string[]): boolean {
  const text = `${job.title} ${job.description}`;
  if (excludedFamily(text)) return false;
  const industrialProfile = profileHasIndustrialFamily(profileSources.join(" "));
  if (!industrialProfile) return true;
  const signals = industrialSignalCount(text);
  return isStrongIndustrialTitle(job.title) || signals >= 2;
}
...
export function buildSearchQueries(sources: string[], max = 6): string[] { const queries: string[] = []; const push = (q: string) => { const v = q.trim(); if (v && !queries.some((x) => normalize(x) === normalize(v))) queries.push(v); }; for (const source of sources) { if (!source?.trim()) continue; const norm = normalize(source); let matched = false; for (const [key, syns] of Object.entries(SYNONYMS)) { if (norm.includes(key.slice(0, 6))) { for (const s of syns.slice(0, 2)) push(s); matched = true; } } if (!matched) push(source.trim()); if (queries.length >= max) break; } return queries.slice(0, max); }

export async function fetchLiveJobs(query: string, limit = 40, extraKeywords: string[] = [], roles: string[] = []): Promise<LiveJob[]> {
  const sources = [query, ...extraKeywords, ...roles].filter(Boolean); const terms = expandKeywords(sources); const industrialProfile = profileHasIndustrialFamily(sources.join(" ")); const searchTerms = buildSearchQueries([...roles, query, ...extraKeywords].filter(Boolean), 6);
  const results = await Promise.all([fromArbeitnow(), ...searchTerms.flatMap((q) => [fromRemotive(q), fromJobicy(q)])]); const seen = new Set<string>(); const scored: Array<{ job: LiveJob; score: number }> = [];
  for (const pool of results) for (const job of pool) { if (!job.url || !job.title || !job.company || seen.has(job.url)) continue; seen.add(job.url); if (!isProfessionallyRelevantLiveJob(job, sources)) continue; const score = relevance(job, terms); if (score < (industrialProfile ? 7 : 3)) continue; scored.push({ job, score }); }
